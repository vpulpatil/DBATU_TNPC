...
public class StudentInfoTPO extends AppCompatActivity {
    ...
    private EditText studentName, studentGender, studentDOB, studentContact, studentEmail, studentAddress, studentAdmissionType,
                     ...
                     studentSGPA4, studentSGPA5, studentSGPA6, studentBacklog, studentExam, password;

    private TextInputLayout passwordTextInputLayout;

    private LinearLayout linearLayout;

    private Button update, edit, addTnpc;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_student_info_tpo);
        ...
        addTnpc.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    private void addTnpcCoordinator() {
        final String name = studentName.getText().toString().trim();
        final String gender = studentGender.getText().toString().trim();
        final String contactnum = studentContact.getText().toString().trim();
        final String email = studentEmail.getText().toString().trim();
        final String rollno = studentRollno.getText().toString().trim();
        final String dept = studentDept.getText().toString().trim();
        String _password = password.getText().toString().trim();

        if (email.equals(firebaseAuth.getCurrentUser().getEmail().trim())){
            Snackbar snackbar = Snackbar.make(linearLayout, "Email already registered", Snackbar.LENGTH_SHORT);
            snackbar.show();
        }
        if (_password.equals("")){
            ...
            firebaseAuth.createUserWithEmailAndPassword(email, _password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                ...
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if (task.isSuccessful()){
                        ...
                        DatabaseReference tnpcReference = ref.child(user_id);

                        tnpcReference.child("Full_Name").setValue(name);
                        tnpcReference.child("Gender").setValue(gender);
                        tnpcReference.child("Contact_Number").setValue(contactnum);
                        tnpcReference.child("Roll_Number").setValue(rollno);
                        tnpcReference.child("Department").setValue(dept);
                        password.setText("");
                        Snackbar snackbar = Snackbar.make(linearLayout, "Added Successfully", Snackbar.LENGTH_SHORT);
                        snackbar.show();
                        progressDialog.dismiss();
                    }else if (task.getException() instanceof FirebaseAuthUserCollisionException) {
                        Snackbar snackbar = Snackbar.make(linearLayout, "User with this email already exist", Snackbar.LENGTH_SHORT);
                        snackbar.show();
                        password.setText("");
                        progressDialog.dismiss();
                    }
    ...
    private void updateInfo() {
        ...
        Float sscf = null, hscf = null, diplomaf = null, sgpa1f = null, sgpa2f = null,sgpa3f = null, sgpa4f = null, sgpa5f = null, sgpa6f = null;
        if (admission.equals("First Year Admission")) {
            sscf = Float.parseFloat(SSC);
            hscf = Float.parseFloat(HSC);
            sgpa1f = Float.parseFloat(sgpa1);
            sgpa2f = Float.parseFloat(sgpa2);
            sgpa3f = Float.parseFloat(sgpa3);
            sgpa4f = Float.parseFloat(sgpa4);
            sgpa5f = Float.parseFloat(sgpa5);
            sgpa6f = Float.parseFloat(sgpa6);
        }
        if (admission.equals("Diploma Admission")) {
            sscf = Float.parseFloat(SSC);
            diplomaf = Float.parseFloat(diplomaPercentage);
            sgpa3f = Float.parseFloat(sgpa3);
            sgpa4f = Float.parseFloat(sgpa4);
            sgpa5f = Float.parseFloat(sgpa5);
            sgpa6f = Float.parseFloat(sgpa6);
        ...
        if (fullName.equals(null) || DOB.equals(null) || contactNumber.equals(null) || email.equals(null) ||
            ...
            Toast.makeText(StudentInfoTPO.this, "All * Fields are Compulsory", Toast.LENGTH_SHORT).show();
        }
    ...
    private void initialize() {
        ...
        password = (EditText) findViewById(R.id.passwordForTnpc);

        //TextInputLaout
        passwordTextInputLayout = (TextInputLayout) findViewById(R.id.passTextInputLayout);
        ...
        edit = (Button) findViewById(R.id.edit_tpo);
        addTnpc = (Button) findViewById(R.id.addTnpc);
        update.setVisibility(View.GONE);
